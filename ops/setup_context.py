...
class CPP_OT_setup_context(Operator):
	bl_idname='cpp.setup_context';bl_label='Setup Context';bl_description='Setup the scene, tool, and data to work with';bl_options={'REGISTER'};bl_translation_context=_A;status:EnumProperty(items=((common.StageStatus.FINISHED.name,'Finished','',0,common.StageStatus.FINISHED.value),(common.StageStatus.CANCELLED.name,'Cancelled','',0,common.StageStatus.CANCELLED.value),(common.StageStatus.ABORTED.name,'Aborted','',0,common.StageStatus.ABORTED.value)),options={'HIDDEN','SKIP_SAVE'})

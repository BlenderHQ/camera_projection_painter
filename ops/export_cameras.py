...
class CPP_OT_export_cameras(Operator,common.IOFileBase_Params,common.IOFile_Params,common.IOTransform_Params,props.intern.rc_csv.RC_CSV_ExportParams,props.intern.rc_xmp.RC_MetadataXMP_Params,props.intern.rc_xmp.RC_MetadataXMP_ExportParams):
	bl_idname='cpp.export_cameras';bl_label='Export Cameras';bl_description='Export camera data';bl_options={'REGISTER','UNDO','PRESET'};bl_translation_context=_B;check_existing:BoolProperty(default=_A,options={_C},translation_context=_B,name='Check Existing',description='Check and warn on overwriting existing files');fmt_locked:BoolProperty(description='',default=_A,options={_C},translation_context=_B,name='Format Locked');open_directory:BoolProperty(default=_A,options={_C},translation_context=_B,name='Open Directory',description='Open directory after export');fmt:EnumProperty(items=((common.IOFormat.RC_IECP.name,'Internal/External camera parameters','',icons.get_id(_D),common.IOFormat.RC_IECP.value),(common.IOFormat.RC_NXYZ.name,'Comma-separated Name, X, Y, Z','',icons.get_id(_D),common.IOFormat.RC_NXYZ.value),(common.IOFormat.RC_NXYZHPR.name,'Comma-separated Name, X, Y, Z, Heading, Pitch, Roll','',icons.get_id(_D),common.IOFormat.RC_NXYZHPR.value),(common.IOFormat.RC_NXYZOPK.name,'Comma-separated Name, X, Y, Z, Omega, Phi, Kappa','',icons.get_id(_D),common.IOFormat.RC_NXYZOPK.value),(common.IOFormat.RC_METADATA_XMP.name,'Metadata (XMP)','',icons.get_id(_D),common.IOFormat.RC_METADATA_XMP.value)),options={_C},translation_context='IOFormat',name='Format');name_source:EnumProperty(items=((common.UnifiedNameOptions.USE_CAMERA_NAME.name,'Use Object Name','Use camera object name as source'),(common.UnifiedNameOptions.USE_CAM_NAME.name,'Use Camera Name','Use camera data name as source'),(common.UnifiedNameOptions.USE_IMAGE_NAME.name,'Use Image Name','Use image data-block name as source'),(common.UnifiedNameOptions.USE_IMAGE_FILEPATH.name,'Use Image File Name','Use image file name as source')),default=common.UnifiedNameOptions.USE_CAMERA_NAME.name,options={_C},translation_context=_B,name='Name Source',description='Name source for export')
	...
	def execute(self,context:Context):
		...
		if done_num_files and done_num_cameras:
			Reports.report_and_log(self,level=logging.INFO,message='Exported {num_cameras:d} camera(s) to {num_files:d} file(s) in {elapsed:.3f} second(s)',msgctxt=msgctxt,num_cameras=done_num_cameras,num_files=done_num_files,elapsed=done_in)

...
_E='CANCELLED'
_D='INTERNAL'
_C='CPP_OT_ensure_canvas'
_B='CPP_OT_canvas_new'
_A='FINISHED'
import logging,re
from.import common
from..import icons
...
class CPP_OT_canvas_new(Operator):
	bl_idname='cpp.canvas_new';bl_label='New';bl_translation_context=_B;bl_options={_D}
...
class CPP_OT_canvas_quick_select(Operator):
	bl_idname='cpp.canvas_quick_select';bl_description='Select image';bl_label='Quick Select';bl_translation_context='CPP_OT_canvas_quick_select';bl_options={_D}
...
class CPP_OT_ensure_canvas(metaclass=common.SetupContextOperator):
	bl_idname='cpp.ensure_canvas';bl_label='Ensure Canvas';bl_translation_context=_C;bl_options={'REGISTER'}
	def draw(self:Operator,context:Context):
		...
		if imapaint.canvas:col.prop(wm_props,'apply_udim_materials_fix')
	...
	def execute(self,context:Context):
		...
		if wm_props.apply_udim_materials_fix:
			ob=context.object

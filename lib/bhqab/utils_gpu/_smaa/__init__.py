...
class SMAA(_common.AABase):
	...
	def draw(self,*,texture:GPUTexture)->_A:
		...
		with fb.bind():fb=gpu.state.active_framebuffer_get();fb.clear(color=(A,A,A,A));self._setup_gpu_state();shader=self._shaders_eval[0];shader.uniform_sampler(_C,texture);shader.uniform_float(_B,viewport_metrics);_common.BatchPreset.ndc_rectangle_tris_P_UV.draw(shader)
		fb_framework_1=self._fb_framework_stage_1;fb=fb_framework_1.get()
		with fb.bind():fb=gpu.state.active_framebuffer_get();fb.clear(color=(A,A,A,A));self._setup_gpu_state();shader=self._shaders_eval[1];shader.uniform_sampler(_D,fb_framework_0.get_color_texture());shader.uniform_sampler(_F,cls._search_texture);shader.uniform_sampler(_E,cls._area_texture);shader.uniform_float(_B,viewport_metrics);_common.BatchPreset.ndc_rectangle_tris_P_UV.draw(shader)
		with gpu.matrix.push_pop():self._setup_gpu_state();shader=self._shaders_eval[2];shader.uniform_sampler(_C,texture);shader.uniform_sampler(_G,fb_framework_1.get_color_texture());shader.uniform_float(_B,viewport_metrics);_common.BatchPreset.ndc_rectangle_tris_P_UV.draw(shader)
	@staticmethod

		...
		match self.tab:
			...
			case'KEYMAP':
				...
				if km:
					...
					if kmi:rna_keymap_ui.draw_kmi([],kc,km,kmi,col,0)
			case'INFO':
				def _intern_show_info_section(*,flag:str)->bool:return bhqab.utils_ui.template_disclosure_enum_flag(layout,item=self,prop_enum_flag=_O,flag=flag)
				for flag in(_I,_M,_N):
					if _intern_show_info_section(flag=flag):text=bhqab.utils_ui.request_localization_from_file(module=ADDON_PKG,langs=langs.LANGS,msgctxt=flag,src=os.path.join(DATA_DIR,f"{flag}.txt"),dst={'uk':os.path.join(DATA_DIR,f"{flag}_uk.txt")});bhqab.utils_ui.draw_wrapped_text(context,layout,text=text,text_ctxt=flag)
				if _intern_show_info_section(flag=_L):update_props:bhqab.utils_ui.PreferencesUpdateProperties=self.update_props;update_props.draw(context,layout)

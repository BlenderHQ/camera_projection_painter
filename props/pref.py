...
class Preferences(AddonPreferences):
	...
	def get_tab_items(self,context:Context)->tuple:
		_tab_items=('APPEARANCE','Appearance','Appearance settings',icons.get_id('appearance'),0),('KEYMAP','Keymap','Keymap settings',icons.get_id(_I),1),('INFO','Info','Information and links',icons.get_id('info'),2);ret=_tab_items
	...
	def draw(self,context:Context):
		...
		match self.tab:
			case'APPEARANCE':
				if bhqab.utils_ui.template_disclosure_enum_flag(layout,item=self,prop_enum_flag=_G,flag=_F):col=layout.column();col.prop(self,_O);col.prop(self,_T);col.prop(self,_X,expand=_B);col.prop(self,_P);scol=col.column();scol.enabled=self.use_previews;scol.prop(self,_S);scol.prop(self,_R);scol.prop(self,_V,expand=_B)
				if bhqab.utils_ui.template_disclosure_enum_flag(layout,item=self,prop_enum_flag=_G,flag='AA'):col=layout.column(align=_B);bhqab.utils_gpu.DrawFramework.ui_preferences(layout,pref=self,attr_aa_method='aa_method',attr_smaa_preset='smaa_preset',attr_fxaa_preset='fxaa_preset',attr_fxaa_value='fxaa_value')
			case'KEYMAP':
				col=layout.column();kc=context.window_manager.keyconfigs.addon;km:_D|KeyMap=kc.keymaps.get('Image Paint')
				if km:
					col.context_pointer_set(_I,km);kmi=self._get_hotkey_entry_item(km,main.CPP_OT_select.bl_idname,_D,_D)
					...
					if kmi:rna_keymap_ui.draw_kmi([],kc,km,kmi,col,0)
			case'INFO':
				def _intern_show_info_section(*,flag:str)->bool:return bhqab.utils_ui.template_disclosure_enum_flag(layout,item=self,prop_enum_flag=_N,flag=flag)
				for flag in(_H,_L,_M):
				...
				if _intern_show_info_section(flag=_K):update_props:bhqab.utils_ui.PreferencesUpdateProperties=self.update_props;update_props.draw(context,layout)

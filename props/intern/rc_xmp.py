...
_D='RC_MetadataXMP_ExportParams'
_C='RC_MetadataXMP_Params'
_B=True
_A='SKIP_SAVE'
...
__all__='RC_METADATA_XMP_PosePrior','RC_METADATA_XMP_CalibrationPrior','RC_METADATA_XMP_Coordinates',_F,'RC_METADATA_XMP_Overwrite',_C,_D,'RC_MetadataXMP_Props','create_props_rc_rotation'
...
class RC_MetadataXMP_Params:rc_metadata_xmp_export_mode:EnumProperty(items=((RC_METADATA_XMP_ExportMode.DO_NOT_EXPORT.name,'Do Not Export','Do not export camera poses'),(RC_METADATA_XMP_ExportMode.DRAFT.name,'Draft','This will treat poses as an imperfect draft to be optimized in the future. The draft mode functions also as a flight log'),(RC_METADATA_XMP_ExportMode.EXACT.name,'Exact','If you trust the alignment absolutely. By choosing this option, you are saying to the application that poses are precise, but the global position, orientation, and scale is not known'),(RC_METADATA_XMP_ExportMode.LOCKED.name,'Locked','This is the same as the exact option with the difference that the camera position and calibration will not be changed, when locked')),default=RC_METADATA_XMP_ExportMode.DRAFT.name,options={_A},translation_context=_F,name=_G,description='Depending on how much you trust your registration, you can select the following options or you can also choose not to export camera poses for Reality Capture (XMP)');rc_metadata_xmp_calibration_group:IntProperty(default=-1,min=-1,options={_A},translation_context=_C,name=_H,description='By defining a group for Reality Capture (XMP) we state that all images in this group have the same calibration properties, e.g. the same focal length, the same principal point. Use "-1" if you do not want to group the parameters');rc_metadata_xmp_distortion_group:IntProperty(default=-1,min=-1,options={_A},translation_context=_C,name=_I,description='By defining a group for Reality Capture (XMP) we state that all images in this group have the same lens properties, e.g. the same lens distortion coefficients. Use "-1" if you do not want to group the parameters');rc_metadata_xmp_in_texturing:BoolProperty(default=_B,options={_A},translation_context=_C,name=_J,description='Whether to use an image to create an object texture for Reality Capture (XMP)');rc_metadata_xmp_in_meshing:BoolProperty(default=_B,options={_A},translation_context=_C,name=_K,description='Whether to use an image to create the object mesh data for Reality Capture (XMP)')
class RC_MetadataXMP_ExportParams:
	rc_metadata_xmp_overwrite_flags:EnumProperty(items=((RC_METADATA_XMP_Overwrite.rc_metadata_xmp_export_mode.name,_G,'Overwrite "Export Mode" parameter for export',0,RC_METADATA_XMP_Overwrite.rc_metadata_xmp_export_mode.value),(RC_METADATA_XMP_Overwrite.rc_metadata_xmp_calibration_group.name,_H,'Overwrite "Calibration Group" parameter for export',0,RC_METADATA_XMP_Overwrite.rc_metadata_xmp_calibration_group.value),(RC_METADATA_XMP_Overwrite.rc_metadata_xmp_distortion_group.name,_I,'Overwrite "Distortion Group" parameter for export',0,RC_METADATA_XMP_Overwrite.rc_metadata_xmp_distortion_group.value),(RC_METADATA_XMP_Overwrite.rc_metadata_xmp_in_texturing.name,_J,'Overwrite "In Texturing" parameter for export',0,RC_METADATA_XMP_Overwrite.rc_metadata_xmp_in_texturing.value),(RC_METADATA_XMP_Overwrite.rc_metadata_xmp_in_meshing.name,_K,'Overwrite "In Meshing" parameter for export',0,RC_METADATA_XMP_Overwrite.rc_metadata_xmp_in_meshing.value)),default=set(),options={_A,'ENUM_FLAG'},translation_context=_D,name='Overwrite',description='What data to overwrite for Reality Capture (XMP)');rc_metadata_xmp_use_calibration_groups:BoolProperty(default=_B,options={_A},translation_context=_D,name='Calibration Groups',description='Select to export the information on the created calibration groups for Reality Capture (XMP)');rc_metadata_xmp_include_editor_options:BoolProperty(default=_B,options={_A},translation_context=_D,name='Include Editor Options',description='Export editor states, e.g. enabled/disabled flags for texturing, meshing, and similar for Reality Capture (XMP)')
	def ui_draw_rc_metadata_xmp_overwrite_props(self,context:Context,layout:UILayout):
		def _intern_draw_item(lay:UILayout,item:RC_METADATA_XMP_Overwrite):row=lay.row(align=_B);col=row.column();col.enabled=item.name in self.rc_metadata_xmp_overwrite_flags;col.prop(self,item.name);col=row.column(align=_B);col.emboss='NONE';col.ui_units_x=1;icon='CHECKBOX_HLT'if item.name in self.rc_metadata_xmp_overwrite_flags else'CHECKBOX_DEHLT';col.prop_enum(self,'rc_metadata_xmp_overwrite_flags',item.name,text='',icon=icon)
...
def create_props_rc_rotation()->RC_MetadataXMP_RotationComponentProps:
	...
	def _eval_item_description(i,j):col,row,sign=mapping[i][j];sign_fmt='negative 'if sign==-1 else'';col_fmt=str(col+1);row_fmt=str(row+1);return f"Corresponds to Blender rotation matrix {sign_fmt}value at column: {col_fmt}, row: {row_fmt}"
	for i in range(3):
		for j in range(3):attr_name=common.matrix_eval_attr_name(i,j);_prop_single_T,_prop_double_T,_prop=double.property_group(attr_name,get=common.remapped_matrix_getter_helper(mapping,i,j),set=common.remapped_matrix_setter_helper(mapping,i,j),translation_context=_E,name=f"Item: {i*3+j+1} (Row: {i+1}, Column: {j+1})",description=_eval_item_description(i,j));_annotations_dict[double.eval_prop_single_name(name=attr_name)]=_prop_single_T;_annotations_dict[double.eval_prop_double_name(name=attr_name)]=_prop_double_T;_properties_dict[attr_name]=_prop
	def _draw(self,context:Context,layout:UILayout):
		for i in range(3):
			row=layout.row(align=_B);row.use_property_split=_B;row.use_property_decorate=False
